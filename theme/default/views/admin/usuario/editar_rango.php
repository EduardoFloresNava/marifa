...
<div class="header">
	<h2 class="title">Editar rango</h2>
</div>
	...
	<div class="control-group{if="$error_imagen"} error{/if}">
		...
		<div class="controls">
			<select name="imagen" id="imagen">
				...
				<option style="background: transparent url({#THEME_URL#}/assets/img/rangos/{$value}) no-repeat;" value="{$value}"{if="$value == $imagen"} selected="selected"{/if}>{$value}</option>
				{/loop}
			</select>
