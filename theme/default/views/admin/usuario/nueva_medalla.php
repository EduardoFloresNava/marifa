	...
	<div class="control-group{if="$error_imagen"} error{/if}">
		...
		<div class="controls">
			<select name="imagen" id="imagen">
				...
				<option style="padding-left: 22px; background: transparent url({#THEME_URL#}/assets/img/medallas/{$value}) no-repeat 2px 0;" value="{$key}"{if="$key == $imagen"} selected="selected"{/if}>{$key}</option>
				{/loop}
			</select>
