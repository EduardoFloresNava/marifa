		...
		<div class="span12">
			...
			<div>
				...
				<div style="margin-left: 30px;">
					<p><a href="{#SITE_URL#}/post/{$value.categoria.seo}/{$value.id}/{$value.titulo|Texto::make_seo}.html">{$value.titulo}</a></p>
					<p>{$value.fecha->fuzzy()} - <a href="{#SITE_URL#}/@{$value.usuario.nick}">@{$value.usuario.nick}</a> - {@Puntos@} {$value.puntos} - {@Comentarios@} {$value.comentarios}<span class="pull-right">{$value.categoria.nombre}</span> <a href="{#SITE_URL#}/buscador/relacionados/{$value.id}">Buscar relacionados</a></p>
				</div>
			</div>
			{else}
			<div class="alert">
