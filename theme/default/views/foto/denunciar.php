	...
	<div class="span12">
		<form method="POST" class="form-horizontal" action="">
			...
			<div class="control-group{if="$error_motivo"} error{/if}">
				...
				<div class="controls">
					<select name="motivo" id="tipo">
						...
						<option value="2" {if="$motivo == 1"}selected="selected"{/if}>La imagen est&aacute; ca&iacute;da</option>
						<option value="3" {if="$motivo == 2"}selected="selected"{/if}>Es racista o irrespetuosa</option>
						<option value="4" {if="$motivo == 3"}selected="selected"{/if}>Contiene informaci&oacute;n personal</option>
						<option value="5" {if="$motivo == 4"}selected="selected"{/if}>Contiene pedofilia</option>
						<option value="6" {if="$motivo == 5"}selected="selected"{/if}>Es gore o asqueros</option>
			...
			<div class="form-actions">
				<button type="submit" name="submit" value="enviar" class="btn btn-large btn-primary">Crear</button> o <a href="/foto/ver/{$foto}">Volver</a>
			</div>
		</form>
	</div>
</div>