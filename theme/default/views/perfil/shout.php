...
<div class="btn-group links-shout">
	...
	<a class="btn active" href="{#SITE_URL#}/perfil/votar_publicacion/{$usuario.nick}/{$shout.id}/0"><i class="icon icon-thumbs-up"></i> {$shout.votos}</a>
		{else}
	<a class="btn" href="{#SITE_URL#}/perfil/votar_publicacion/{$usuario.nick}/{$shout.id}/1"><i class="icon icon-thumbs-up"></i> {$shout.votos}</a>
		{/if}
	{else}
	...
	<a class="btn active" href="{#SITE_URL#}/perfil/favorito_publicacion/{$usuario.nick}/{$shout.id}/0"><i class="icon icon-star"></i> {$shout.favoritos}</a>
		{else}
	<a class="btn" href="{#SITE_URL#}/perfil/favorito_publicacion/{$usuario.nick}/{$shout.id}/1"><i class="icon icon-star"></i> {$shout.favoritos}</a>
		{/if}
	{else}
	...
	{else}
		{if="Usuario::is_login()"}
	<a class="btn" href="{#SITE_URL#}/perfil/compartir_publicacion/{$usuario.nick}/{$shout.id}"><i class="icon icon-retweet"></i> {$shout.compartido}</a>
		{else}
...
<div class="comentarios">
	...
	<div class="comentario clearfix" id="c-{$value.id}">
		<a href="{#SITE_URL#}/perfil/index/{$value.usuario.nick}"><img class="thumbnail pull-left" src="{function="Utils::get_gravatar($value.usuario.email, 64, 64)"}" /></a>
		<div class="content">
			<h4 class="title"><a href="{#SITE_URL#}/perfil/index/{$value.usuario.nick}">{$value.usuario.nick}</a><small><i class="icon icon-time"></i> {$value.fecha->fuzzy()}</small></h4>
...
<form action="{#SITE_URL#}/perfil/comentar_publicacion/{$usuario.nick}/{$shout.id}/" method="POST">
