	...
	<div class="cuerpo">
		...
		<div class="contenido">
			<div class="wrapper">
				...
				{else}
				{@Ha editado el comentario de@} <a href="{#SITE_URL#}/@{$suceso.comentario_usuario.nick}">{$suceso.comentario_usuario.nick}</a> {@en la foto@} <a href="{#SITE_URL#}/foto/{$suceso.foto.categoria.seo}/{$suceso.foto.id}/{$suceso.foto.titulo|Texto::make_seo}.html">{$suceso.foto.titulo}</a>.
				{/if}
			</div>
		</div>
	</div>
