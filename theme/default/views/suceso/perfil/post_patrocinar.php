<div class="suceso clearfix">
	...
	<div class="cuerpo">
		...
		<div class="contenido">
			<div class="wrapper">
				{@Su post @} <a href="{#SITE_URL#}/post/{$suceso.post.categoria.seo}/{$suceso.post.id}/{$suceso.post.titulo|Texto::make_seo}.html">{$suceso.post.titulo}</a> {if="$suceso.tipo"}{@ha sido patrocinado@}{else}{@ya no está patrocinado@}{/if}.
			</div>
		</div>
	</div>
</div>