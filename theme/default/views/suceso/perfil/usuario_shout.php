<div class="suceso clearfix">
	...
	<a href="{#SITE_URL#}/perfil/index/{$suceso.shout.usuario.nick}" class="usuario"><img class="thumbnail" src="{function="Utils::get_gravatar($suceso.shout.usuario.email, 50, 50)"}" alt="{$suceso.shout.usuario.nick}" /></a>
	{else}
	...
	<div class="cuerpo">
		<div class="cabecera">
			{if="$actual.id == $suceso.usuario.id"}
			<a href="{#SITE_URL#}/perfil/index/{$suceso.shout.usuario.nick}">{$suceso.shout.usuario.nick}</a>
			{else}
		...
		<div class="contenido">
			<div class="wrapper">
				{if="$actual.id == $suceso.usuario.id"}
					...
					{/if}
				{else}
				{@Ha publicado un@} <a href="{#SITE_URL#}/perfil/publicacion/{$suceso.shout.usuario.nick}/{$suceso.shout.id}">{@shout@}</a> {@en el perfil de @} <a href="{#SITE_URL#}/perfil/index/{$actual.nick}">{$actual.nick}</a>
				{/if}
			</div>
		</div>
		{if="$actual.id == $suceso.usuario.id"}
