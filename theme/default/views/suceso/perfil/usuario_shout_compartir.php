	...
	<div class="cuerpo">
		...
		<div class="contenido">
			...
			{if="$actual.id == $suceso.usuario.id"}
				{@Ha compartido a través de @} <a href="{#SITE_URL#}/@{$suceso.usuario_comparte.nick}">{$suceso.usuario_comparte.nick}</a> {@el@} <a href="{#SITE_URL#}/@{$suceso.shout.usuario.nick}/publicacion/{$suceso.shout.id}/">{@shout@}</a>: {$suceso.shout.mensaje}
			{else}
				{if="$suceso.shout.usuario.id == $suceso.usuario_comparte.id"}
				{@Ha compartido el@} <a href="{#SITE_URL#}/@{$suceso.shout.usuario.nick}/publicacion/{$suceso.shout.id}/">{@shout@}</a> {@de@} <a href="{#SITE_URL#}/@{$suceso.shout.usuario.nick}">{$suceso.shout.usuario.nick}</a>.
				{else}
				{@Ha compartido el@} <a href="{#SITE_URL#}/@{$suceso.shout.usuario.nick}/publicacion/{$suceso.shout.id}/">{@shout@}</a> {@de@} <a href="{#SITE_URL#}/@{$suceso.shout.usuario.nick}">{$suceso.shout.usuario.nick}</a> {@a través de@} <a href="{#SITE_URL#}/@{$suceso.usuario_comparte.nick}">{$suceso.usuario_comparte.nick}</a>.
				{/if}
			{/if}
			</div>
		</div>
