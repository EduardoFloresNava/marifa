<div class="suceso">
	...
	<div class="contenido">
		...
		{else}
		<a href="/perfil/index/{$suceso.u_voto.nick}">{$suceso.u_voto.nick}</a> {@vot&oacute;@} {if="$suceso.voto"}<span class="label label-success">{@positivamente@}</span>{else}<span class="label label-important">{@negativamente@}</span>{/if} {@el comentario de@} <a href="/perfil/index/{$suceso.u_comentario.nick}">{$suceso.u_comentario.nick}</a> {@en tu post@} <a href="/post/index/{$suceso.post.id}">{$suceso.post.titulo}</a>.
		{/if}
	</div>
